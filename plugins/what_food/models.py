...
class ScoreRecord(Model):
    ...
    user_id: Mapped[int] = mapped_column(BigInteger)  # AI 设为 -200
...
class UserBan(Model):
    """用户禁用记录模型"""
    __tablename__ = "whatfood_user_ban"
    __table_args__ = (UniqueConstraint('user_id', 'group_id'),)  # 每个群的用户只能有一条禁用记录

    id: Mapped[int] = mapped_column(primary_key=True)
    user_id: Mapped[int] = mapped_column(BigInteger, index=True)
    group_id: Mapped[int] = mapped_column(BigInteger, index=True)
    reason: Mapped[str | None] = mapped_column(default=None, nullable=True)  # 禁用原因
    ban_time: Mapped[int] = mapped_column()  # 禁用时长（秒）
    create_time: Mapped[int] = mapped_column(default=lambda: int(time.time()))  # 禁用创建时间
    end_time: Mapped[int] = mapped_column()  # 禁用结束时间
