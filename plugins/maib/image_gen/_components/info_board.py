...
class MaiChartInfoBoard:
    ...
    def _metadata(cls, maidata: MaiData, ms: MS = MS(), ui_code: UICode = UICode.JP) -> Image.Image:
        ...
        drawer.rounded_rect(image_outline_width, image_outline_width, image_size, image_size,
                            radius=5, fill=None, outline=GRAY, width=image_outline_width, closed_interval=False)
        ...
        cap_font = FontManager.font(FontCode.JBMono_Bold, size=ms.x(image_size*0.06))
        cap_tds = TextDrawStyle(fill=genre_style.fill, anchor='mm', font=cap_font)
        drawer.capsule(margin, cover_y, cap_width, cap_height, fill=WHITE, outline=None, outline_width=0)
        drawer.text(margin + cap_width / 2, cover_y + cap_height / 2,
                    text=f"ID: {str(maidata.shortid).replace('0', 'O'):>6}", tds=cap_tds)
        drawer.capsule(margin + half_size, cover_y, cap_width, cap_height, fill=WHITE, outline=None, outline_width=0)
        drawer.text(margin + half_size + cap_width / 2, cover_y + cap_height / 2,
                    text=f"BPM:{str(maidata.bpm).replace('0', 'O'):>6}", tds=cap_tds)
...
draw_info_board = MaiChartInfoBoard.board
