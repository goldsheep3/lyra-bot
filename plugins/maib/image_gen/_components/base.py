...
class TextDrawStyle:
    ...
    def new(self, **kwargs) -> 'TextDrawStyle':
        """返回一个新的 TextDrawStyle 对象，属性值由 kwargs 覆盖"""
        return TextDrawStyle(
            ...
            stroke=kwargs.get('stroke', self.stroke),
            stroke_width=kwargs.get('stroke_width', self.stroke_width),
            shadow=kwargs.get('shadow', self.shadow),
            shadow_width=kwargs.get('shadow_width', self.shadow_width),
            shadow2=kwargs.get('shadow2', self.shadow2),
            shadow2_width=kwargs.get('shadow2_width', self.shadow2_width),
            shadow2_offset=kwargs.get('shadow2_offset', self.shadow2_offset)
        )
...
class Drawer:
    ...
    def rounded_rect(self, x: float, y: float, w: float, h: float, fill: Optional[str], 
                    radius: float = 0, outline: Optional[str] = None, width: float = 0,
                    *, closed_interval: bool = True, corners: Optional[tuple[bool, bool, bool, bool]] = None):
        """绘制圆角矩形"""
        if radius <= 0:
            self.draw.rectangle(
                self.ms.size(x, y, w, h, closed_interval=closed_interval),
                fill=fill,
    ...
    def capsule(self, x: float, y: float, w: float, h: float, fill: Optional[str],
        ...
        """绘制胶囊形状"""
        xywh = self.ms.size(x, y, w, h, closed_interval=True)
        radius = round(min(*self.ms.xy(w, h)) / 2)
        self.draw.rounded_rectangle(
            xywh, radius=radius, fill=fill,
    ...
    def get_mask(cls, w: float, h: float, radius: float, *, ms: MS = MS()) -> Image.Image:
        ...
        drawer.rounded_rect(0, 0, w, h, fill='#FFFFFF', radius=radius, closed_interval=False)
        return mask

    @classmethod
    def get_mask2(cls, w: float, h: float, radius: float, *, ms: MS = MS()) -> Image.Image:
        """获取圆角遮罩"""
        mask = Image.new('L', ms.xy(w, h), 0)
        drawer = cls(mask, ms=ms)
    ...
    @classmethod
    def masked(cls, img: Image.Image, radius: float, *, ms: MS = MS()) -> Image.Image:
        """在原图上应用遮罩"""
        mask = Image.new('L', img.size, 0)
        draw = ImageDraw.Draw(mask)
        draw.rounded_rectangle((0, 0, img.size[0]-1, img.size[1]-1), radius=ms.x(radius), fill=255)
        img.putalpha(mask)
        mask.close()
        return img
