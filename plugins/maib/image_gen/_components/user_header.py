...
class UserHeaderBadge:
    ...
    def dxrating(cls, dxrating: int, cirp_frame: bool = True, ms: MS = MS()) -> Image.Image:
        ...
        if img is None:
            ...
            x, y, w, h = 0.5, 0.5, cls.dxrating_width-1, cls.dxrating_height-1
            drawer.rounded_rect(x, y, w, h, radius=1, fill=WHITE,
                                outline=GRAY, width=0.5, closed_interval=False)
        ...
        font = FontManager.font(FontCode.MiSans_Demibold, size=ms.x(8.5))
        for i, digit in enumerate(dxra_str[::-1]):
            drawer.text(62 - 6 * i, 7.5, text=digit, tds=TextDrawStyle(
                fill="#FCC916", anchor="mm", font=font, stroke="#333333", stroke_width=0.5
            ))
