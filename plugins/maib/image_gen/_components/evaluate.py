...
from ..style import COMBO_TYPE, SYNC_TYPE, get_combo_style, get_sync_style
from .base import TextDrawStyle, Drawer


_EVALUTATE_TYPE = COMBO_TYPE | SYNC_TYPE
...
class EvaluateBadge:
    ...
    def _evaluate_badge(cls, evaluate: _EVALUTATE_TYPE, evaluate_type: _EVALUATE_INPUT_TYPE,
        ...
        if evaluate_type == 'combo' and isinstance(evaluate, COMBO_TYPE):
            style = get_combo_style(combo=evaluate, short=(display == 'short'), is_cn_all=(display == 'cn'))
        elif evaluate_type == 'sync' and isinstance(evaluate, SYNC_TYPE):
    ...
    @classmethod
    def combo(cls, combo: COMBO_TYPE, mini: bool = False, ui_code: UICode = UICode.JP, ms: MS = MS()) -> Image.Image:
        """绘制连击徽章"""
        return cls._evaluate(evaluate=combo, evaluate_type='combo', mini=mini, ui_code=ui_code, ms=ms)

    @classmethod
    def sync(cls, sync: SYNC_TYPE, mini: bool = False, ui_code: UICode = UICode.JP, ms: MS = MS()) -> Image.Image:
        """绘制同步徽章"""
