...
class MS:
    ...
    def size(self, mpx_x: float, mpx_y: float, mpx_w: float, mpx_h: float,
             *, closed_interval: bool = True) -> tuple[int, int, int, int]:
        """计算 `x`,`y`,`x+w`,`y+h` 坐标 px 值"""
        cha = 1 if closed_interval else 0
        return self.x(mpx_x), self.x(mpx_y), self.x(mpx_x + mpx_w) - cha, self.x(mpx_y + mpx_h) - cha
