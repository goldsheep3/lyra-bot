...
def _user_header_board(inner_width: int, dxrating: int, server: Server,
                       ...
                       ms: MS = MS(), ui_code: UICode = UICode.JP) -> Optional[Image.Image]:
    """生成用户信息看板"""
    header_h = 32
    board_title = Image.new("RGBA", ms.xy(inner_width, header_h), EMP_COLOR)
    du1 = DrawUnit(board_title, multiple=ms, ui_code=ui_code)
...
def draw_info_box(maidata: MaiData, server: Server, maiuser: Optional[MaiUser] = None,
    ...
    du1 = DrawUnit(board1, multiple=ms, ui_code=ui_code)
    ...
    if maidata.genre:
        genre_text, genre_fill = get_genre(maidata.genre, ui_code=ui_code)
    ...
    if maidata.aliases:
        ...
        du2 = DrawUnit(board2, multiple=ms, ui_code=ui_code)
    ...
    for diff, chart in maidata.charts.items():
        ...
        chart_img = func(chart=chart, cabinet_dx=maidata.is_cabinet_dx, server=server, ms=ms, ui_code=ui_code)
    ...
    board_last = ImageUnit.copyright_bar(width=all_width, ms=ms, ui_code=ui_code)
...
def draw_b50(b35_entries: list[tuple[MaiData, int]], b15_entries: list[tuple[MaiData, int]],
    ...
    """绘制 mai_b50 看板"""
    margin = 10
    temp_box = ImageUnit.mini_box(None, 0, Server.JP, ms=ms, ui_code=ui_code)
    ...
    try:
        ...
        b35_imgs = [
            ImageUnit.b50_box(
                ...
                is_b15=False, ms=ms, ui_code=ui_code,
        ...
        b15_imgs = [
            ImageUnit.b50_box(
                maidata=maidata, difficulty=difficulty, server=server, current_version=0, index=index,
                is_b15=True, ms=ms, ui_code=ui_code,
                shared_zip=shared_zip_handles.get(maidata.zip_path) if maidata.zip_path else None,
            )
    ...
    board_last = ImageUnit.copyright_bar(width=width, ms=ms, ui_code=ui_code)
